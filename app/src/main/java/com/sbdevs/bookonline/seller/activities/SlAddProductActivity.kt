...
import com.sbdevs.bookonline.databinding.ActivitySlAddProductBinding
import com.sbdevs.bookonline.fragments.LoadingDialog
import com.sbdevs.bookonline.seller.FixedPriceClass
import com.sbdevs.bookonline.seller.adapters.NewUploadImageAdapter
...
class SlAddProductActivity : AppCompatActivity(), NewUploadImageAdapter.MyOnItemClickListener {
    private lateinit var binding:ActivitySlAddProductBinding
    ...
    private var conditionMandatory: Boolean = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivitySlAddProductBinding.inflate(layoutInflater)
        ...
        val lay21 = binding.lay3
        val lay3 = binding.lay4
        ...
        productReturnRadioTogole = binding.lay3.productReturnToggle





        val recyclerView = binding.lay5.uploadImageRecycler
            ...
            registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result: ActivityResult ->
                ...
                when (resultCode) {
                    Activity.RESULT_OK -> {
                        ...
                        binding.lay5.textView44.text = "${uriList.size}/3"
                        if (uriList.size >0){
                            binding.lay5.placeHolderImage.visibility = inVisible
                            binding.lay5.recyclerContainer.visibility = visible
                        }else{
                            binding.lay5.placeHolderImage.visibility = visible
                            binding.lay5.recyclerContainer.visibility = inVisible
                        }
        ...
        binding.lay5.selectImageBtn.setOnClickListener {
        ...
        val indicator = binding.lay5.recyclerviewPagerIndicator
...
//                TODO("Not yet implemented")
    ...
    override fun onStart() {
        ...
        binding.lay3.newRadioButton1.text = "New"
        lay2.bookStateToggle.setOnCheckedChangeListener { group, checkedId ->
            ...
            when (checkedId) {
                R.id.new_radioButton1 -> {
                    ...
                    conditionMandatory = false
                    lay2.bookConditionLayout.visibility = gone
                }

                R.id.used_radioButton, R.id.refurb_radioButton -> {
                    printDateInput.editText?.hint = "Printed Year(if any)"
                    printDateMandatory = false
                    conditionMandatory = true
                    lay2.bookConditionLayout.visibility = visible
                }
            }
        }
        ...
        binding.lay2.lay2.hideAndShoWText.visibility = gone
        ...
        for (index in 0 until chipGroup.childCount) {
            ...
            chip.setOnCheckedChangeListener { view, isChecked ->
                ...
                if (view.id == R.id.categoryChip37) {
                    ...
                    binding.lay4.textView36.visibility = View.VISIBLE
                }
    ...
    private fun checkType(): Boolean {
        return if (bookStateRadio == null) {
            binding.lay3.bookStateLayout.backgroundTintList =
    ...
    private fun checkCondition(): Boolean {
        return if (bookConditionRadio == null) {
            binding.lay3.bookConditionLayout.backgroundTintList =
                AppCompatResources.getColorStateList(this, R.color.red_50)
            false
        } else {
            binding.lay3.bookConditionLayout.backgroundTintList =
    ...
    private fun checkCategory(): Boolean {
        val categoryContainer = binding.lay4.categoryLayout
    ...
    private fun checkProductReturnState(): Boolean {
        return if (productReturnRadio == null) {
            binding.lay3.returnContainer.backgroundTintList =
                AppCompatResources.getColorStateList(this, R.color.red_50)
            false
        } else {
            binding.lay3.returnContainer.backgroundTintList =
    ...
    private fun checkProductImage(): Boolean {
        val selectBtn = binding.lay5.selectImageBtn
        return if (uriList.size == 0) {
            selectBtn.backgroundTintList =
                AppCompatResources.getColorStateList(this, R.color.red_a700)
            binding.lay5.textView44.text = "No image selected"
            selectBtn.requestFocus()
            false
        } else {
            if (uriList.size > 3) {
                val message = binding.lay5.textView44
                ...
                selectBtn.backgroundTintList =
                    AppCompatResources.getColorStateList(this, R.color.red_700)
                false
            } else {
                selectBtn.backgroundTintList =
    ...
    private fun checkAllDetails(v: View?) {
        ...
        if (!checkName() or !checkPublisher() or !checkWriter() or !checkLanguage() or !checkPageCount()
            ...
            or !checkPrice() or !checkType() or !checkCategory()
    ...
    private fun addNewProduct(documentName: String) {
        try {
            ...
            if (bookConditionRadio == null){
                addProductMap["book_condition"] = "new"
            }else{
...
//                                binding.lay4.textView44.text = "${uriList.size} and ${downloadUriList.size} and itaretion $i"
    ...
    private suspend fun updateProductImageLIST(productID: String, uriList: MutableList<String>) {
        ...
        firebaseFirestore.collection("PRODUCTS").document(productID).update(allMap)
            .addOnSuccessListener {
                ...
                Toast.makeText(this, "Product is successfully added", Toast.LENGTH_LONG)
                    .show()
    ...
    private fun tagsToList(): MutableList<String> {
        ...
        val bookCondition = if (bookConditionRadio == null){
            "new_condition"
        }else{
            bookConditionRadio?.tag.toString().trim()
        }
    ...
    private fun autoTagging() {
        if (!checkName() or !checkPublisher() or !checkWriter() or !checkLanguage() or !checkCategory() or !checkType() ) {
            Toast.makeText(this,"Fill out the required fields",Toast.LENGTH_LONG).show()
            return
        } else {
